function loadModel(live2d, callback) {
	var modelName = live2d.modelNames[Math.floor(Math.random() * live2d.modelNames.length)]
	var modelPath = live2d.baseModelPath + modelName + '/' + modelName + '.model3.json'
	var ajax = null
	...
	var touch_bodyIndex
	var touch_headIndex
	var touch_specialIndex
	function loadMotions(motions) {
		var motionCount = 0
		if (motions.length > 0) {
			for (var i = 0; i < motions.length; i++) {
				PIXI.loader.add('motion' + (motionCount + 1), modelPath.substr(0, modelPath.lastIndexOf('/') + 1) + motions[i].File, {
	...
	function setModel(model) {
		var app
		var canvas = document.querySelector(live2d.tag_target)
		var view = canvas.querySelector('canvas')
		if (app != null) {
			app.stop()
		}
		app = new PIXI.Application(live2d.modelWidth, live2d.modelHight, { transparent: true, view: view })
		...
		var onResize = function (event) {
			...
			var width = live2d.modelWidth
			var height = live2d.modelHight
			app.view.style.width = width + 'px'
			app.view.style.height = height + 'px'
			app.renderer.resize(width, height)
			model.position = new PIXI.Point(live2d.modelWidth / 2 + live2d.model_x, live2d.modelHight / 2 + live2d.model_y)
			model.scale = new PIXI.Point(live2d.scale, live2d.scale)
	...
	function setMouseTrick(model, app, canvas, motions) {
		...
		var center_x = live2d.modelWidth / 2 + rect.left,
			center_y = live2d.modelHight / 2 + rect.top
		...
		app.ticker.add(function (deltaTime) {
			...
			;(center_x = live2d.modelWidth / 2 + rect.left), (center_y = live2d.modelHight / 2 + rect.top)
